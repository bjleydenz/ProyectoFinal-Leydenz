...
const ThankYou = () => {
  ...
  const [client, setClient] = useState(null); // Estado para los datos del cliente
        ...
        <>
          ...
          <ul>
            {order.items.map((item) => (
              <li key={item.id}>
                <div style={{float: 'left' }}><img src={item.image} alt={item.name} style={{ width: 200, marginRight: 10}} /></div>
                <div style={{float: 'right' }}>{item.name} - {item.quantity} x ${item.price} = ${item.total}</div>
          ...
          {}
          <button onClick={handlePrint} style={{ marginTop: '20px', padding: '10px 20px', backgroundColor: '#4CAF50', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
            Imprimir Pantalla
          </button>
        </>
      ) : (
